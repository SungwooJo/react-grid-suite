...
export function cloneLayoutItem(layoutItem: LayoutItem): LayoutItem {
  return {
    ...
    // added by swjo
    ig: layoutItem.ig,
    igItems: layoutItem.igItems || [],
    igLayout: layoutItem.igLayout || []
  };
}
 ...
 * Switch two elements for Template Grid View
 *
 * @param  {Array}      layout Full layout to modify.
 * @param  {LayoutItem} l      element to move.
 * @param  {Number}     [x]    X position in grid units.
 ...
 *                                     being dragged/resized by th euser.
 */
export function switchElement(layout: Layout, l: LayoutItem, x: ?number, y: ?number, oldDragItem: LayoutItem, originItem: LayoutItem): Layout {

  var movedItemKey = l.i;
  var originItemKey = originItem.i;

  var movedItemIndex = null;
  var originItemIndex = null;
  ...
  layout.forEach((l, i) => {
    ...
    else if (l.i === originItemKey) {
      originItemIndex = i;
    }
  });

  if ((movedItemIndex != null) && (originItemIndex != null)) {
    var tempLayoutKey = layout[movedItemIndex].i;
    layout[movedItemIndex].i = layout[originItemIndex].i;
    layout[originItemIndex].i = tempLayoutKey;
  }
 ...
 * @param  {Number} targetRect targeted rectangle
...
export function calcXY(props, top: number, left: number, targetRect: object): {x: number, y: number} {
  ...
  if (targetRect) {
    x = Math.floor(((left - targetRect.left) - margin[0]) / (colWidth + margin[0]));
    y = Math.floor(((top - targetRect.top) - margin[1]) / (rowHeight + margin[1]));
  }
  else {
  ...
  // Capping
  x = Math.max(Math.min(x, cols - (w || 1)), 0);
  y = Math.max(Math.min(y, maxRows - (h || 1)), 0);
 ...
 */
export function calcWH(props, {height, width}: {height: number, width: number}): {w: number, h: number} {
  const {margin, containerWidth, maxRows, cols, rowHeight, x, y} = props;
